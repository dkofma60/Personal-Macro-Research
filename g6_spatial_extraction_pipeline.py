...
OUTPUT_DIR = ROOT / "g6_extraction_output"
CACHE_DIR = OUTPUT_DIR / "_cache"
PIPELINE_VERSION = "cleanup-v3-20260724"
RENDER_DPI = 240
...
def render_and_ocr(pdf_path):
    ...
    cache = CACHE_DIR / PIPELINE_VERSION / pdf_sha
...
def date_rows(
    ...
    possible_start_years = range(max(1960, release_date.year - 12), release_date.year + 1)
    year_scores = []
    for start_year in possible_start_years:
        ...
        for index, row in enumerate(deduplicated):
            ...
            options = [
                ...
                if max(1960, release_date.year - 12)
                <= option["year"]
                <= release_date.year + 1
            ]
            if options:
                score += min(
                    0.0 if option["year"] == generated_year else option["weight"]
                    for option in options
                )
        if generated:
            last_date = date(generated[-1], deduplicated[-1]["month"], 1)
            preferred_last = date(
        ...
        year_scores.append((score, start_year, generated))
    _, _, generated_years = min(year_scores, default=(0, release_date.year, []))
    for row, resolved_year in zip(deduplicated, generated_years):
        ...
        if strong_years and resolved_year not in strong_years:
            row["observation_date_status"] = "date_alignment_error"
            row["observation_date_source"] = "recognized_year_conflicts_with_sequence"
...
def targeted_cell_candidates(
    ...
    for variant, psm, variant_image, whitelist in variants:
        ...
        key = hashlib.sha1(
            (
                f"{PIPELINE_VERSION}:{bbox}:{variant}:{psm}:"
...
def cleanup_report(
    ...
    lines = [
        ...
        f"- Baseline metrics: `{(OUTPUT_DIR / '_cleanup_baseline_metrics.json').resolve()}`",
...
def main():
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    version_cache = CACHE_DIR / PIPELINE_VERSION
    ...
    baseline_path = OUTPUT_DIR / "_cleanup_baseline_metrics.json"
